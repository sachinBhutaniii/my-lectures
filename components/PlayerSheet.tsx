...
export default function PlayerSheet() {
  ...
  const [dragY, setDragY] = useState(0); // px down from fully-expanded (0 = open, large = closed)
  ...
  // Back gesture closes expanded overlay
  useBackClose(expanded, () => setExpanded(false));

  // ── Drag handlers (on drag handle when expanded) ────────────────────────
  ...
  const onHandleTouchMove = useCallback((e: React.TouchEvent) => {
    const y = e.touches[0].clientY;
    if (Math.abs(y - touchStartY.current) > 4) moved.current = true;
    setDragY(Math.max(0, y - touchStartY.current));
    lastTouchY.current = y;
    lastTouchT.current = Date.now();
  }, []);
  ...
  const onHandleTouchEnd = useCallback((e: React.TouchEvent) => {
    setDragging(false);
    const vy = (e.changedTouches[0].clientY - lastTouchY.current) /
               Math.max(1, Date.now() - lastTouchT.current);
    const viewH = window.visualViewport?.height ?? window.innerHeight;
    if (!moved.current || vy > 0.5 || dragY > viewH * 0.38) {
      setExpanded(false);
    } else {
      setDragY(0); // snap back open
    }
  }, [dragY]);

  // ── Mini bar tap / swipe-up handler ─────────────────────────────────────
  ...
  const onMiniTouchMove = useCallback((e: React.TouchEvent) => {
    if (Math.abs(e.touches[0].clientY - touchStartY.current) > 6) moved.current = true;
  }, []);

  const onMiniTouchEnd = useCallback((e: React.TouchEvent) => {
    const dy = touchStartY.current - e.changedTouches[0].clientY; // positive = swipe up
    if (!moved.current || dy > 20) setExpanded(true);
  }, []);
  ...
  return (
    <>
      {/* ── Backdrop ── */}
      <div
        className="fixed inset-0 z-[59] bg-black/60 transition-opacity duration-300"
        style={{ opacity: expanded && !dragging ? 1 : 0, pointerEvents: expanded ? "auto" : "none" }}
      ...
      <div
        ...
        style={{
          ...
          pointerEvents: expanded || dragging ? "auto" : "none",
        }}
      >
        {/* Drag handle */}
        <div
        ...
        <div className="flex-1 overflow-y-auto px-6 pb-8 flex flex-col gap-5">
          ...
          {/* Title */}
          <p className="text-white text-lg font-semibold leading-snug flex-shrink-0">{lecture.title}</p>
          ...
          <div className="flex items-center justify-center gap-10 flex-shrink-0">
            <button onClick={() => skip(-10)} className="text-gray-400 active:text-white active:scale-90 transition-all">
          ...
          {/* Open full lecture page */}
          <button
            onClick={() => { setExpanded(false); setTimeout(() => router.push(`/${lecture.id}`), 400); }}
      ...
      >
        <div className="mx-auto w-full max-w-4xl xl:max-w-6xl">
          ...
          <div className="bg-[#1a1208] border-t border-orange-900/30 px-3 flex items-center gap-3" style={{ height: `${MINI_H - 2}px` }}>
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-gray-500 text-[11px] mt-0.5">{fmt(currentTime)}{duration > 0 ? ` / ${fmt(duration)}` : ""}</p>
            </div>
            <button
